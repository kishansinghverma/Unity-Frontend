...
import { PhonepeItemProps } from '../../../engine/models/props';


export const PhonePeItem: FC<PhonepeItemProps> = (item) => {
  ...
  return (
    <div
      ...
      onClick={() => item.selectedItem.set(item._id)} // Tooltip for accessibility
    >
      ...
      <div className={`flex items-center space-x-3`}>
        ...
        <div className={`flex flex-col items-end flex-shrink-0 ml-auto text-right`}>
          ...
          <div className={`flex items-center text-xs font-semibold whitespace-nowrap ${currentDateTimeColor}`}>
            ...
            <span className="opacity-90">{getDateComponent(item.date).time}</span>
          </div>
        </div>
      </div>
    </div>
